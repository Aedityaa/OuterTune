...
fun OuterTuneTheme(
    ...
    val colorScheme = remember(darkTheme, pureBlack, themeColor, highContrastCompat) {
        if (themeColor == DefaultThemeColor && Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            ...
            // Return the modified scheme if highContrastCompat is true, otherwise return baseScheme
            if (highContrastCompat) {
                baseScheme.copy(
                    secondaryContainer = baseScheme.surfaceContainerHigh,
            ...
            // Fallback for custom colors or older Android versions
            SchemeTonalSpot(Hct.fromInt(themeColor.toArgb()), darkTheme, 0.0)
                .toColorScheme()
                .pureBlack(darkTheme && pureBlack)
        }
